...
import SitnikovNN_Al_WithSimulations
...
if __name__ == '__main__':
    for seed in [1]:
        ...
        if train:
            print("Training Al")
            SitnikovNN_Al_WithSimulations.train(seed, lr, batchSize, epochMax, startingTrainingSize, generateAmount,
                                                generatingEpoch, generatingAfterEpoch, modelName, c)

            print("Training NoAl")
            SitnikovNN_NoAL.train(seed, lr, batchSize, epochMax, startingTrainingSize, generateAmount,
                                  generatingEpoch, generatingAfterEpoch, modelName)

            print("Training NoAl with Al dataset")
            SitnikovNN_NoAL.train(seed, lr, batchSize, epochMax, startingTrainingSize, generateAmount,
                                  generatingEpoch, generatingAfterEpoch, modelName,
                                  fromDataset=f"Datasets/AL_model={modelName}_seed={seed}_train="
        ...
        model4 = SitnikovModels.models[modelName]()
        model4.to(device)
        model4.load_state_dict(torch.load(f"Models/NoAL_model={modelName}_seed={seed}_train="
                                          f"{startingTrainingSize + generatingTotal * generateAmount}"
                                          f"_lr={lr}_batchSize={batchSize}_fromAlDataset.pt"))
        ...
        model3.load_state_dict(torch.load(f"Models/BestNoAL_model={modelName}_seed={seed}_train="
                                          f"{startingTrainingSize + generatingTotal * generateAmount}"
                                          f"_lr={lr}_batchSize={batchSize}.pt"))
        model5 = SitnikovModels.models[modelName]()
        model5.to(device)
        model5.load_state_dict(torch.load(f"Models/BestNoAL_model={modelName}_seed={seed}_train="
                                          f"{startingTrainingSize + generatingTotal * generateAmount}"
                                          f"_lr={lr}_batchSize={batchSize}_fromAlDataset.pt"))

        models = [model, model1, model4, model2, model3, model5]
        ...
        for i in range(len(models)):
            lineStyle = "dotted" if i // 3 == 0 else "solid"
            plt.plot(1 - totTns[i], totTps[i], linestyle=lineStyle, color=colors[i % 3])
        ...
        plt.legend(handles=[red_patch, blue_patch, green_patch, line1, line2], loc="lower right")
        ...
        for i in range(len(models)):
            lineStyle = "dotted" if i // 3 == 0 else "solid"
            plt.plot(thresholds, totAccuracies[i], linestyle=lineStyle, color=colors[i % 3])

        plt.legend(handles=[red_patch, blue_patch, green_patch, line1, line2], loc="lower center")
